<template>
  <div class="search-demo">
    <div class="header">
      ...
      <div>
        <van-search type="text" v-model="value" placeholder="请搜索" @input="inputHandel" />
      </div>
      <div>
        <span @click="SearchDemo">搜索</span>
      </div>
    </div>
    <ul>
      <div class="hoisy">
        ...
        <ul>
          <li v-for="(list,i) in hoisylist" :key="i" @click="HoisyDemo(list)">{{list}}</li>
        </ul>
      </div>
      <div class="hoisy">
        <h2>
          <i class="iconfont">&#xe889;</i> 搜索内容
        </h2>
      </div>

      <div class="list">
        <li v-for="(item,index) in searchList" :key="index" @click="Getid(item.id,item.title)">
          <i class="iconfont">&#xe634;</i>
