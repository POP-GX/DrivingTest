<template>
  <div class="refuse-class">
    <div class="banner">
      ...
      <div>
        <span>
          <input type="text" placeholder="请搜索" @keyup.enter="Search" v-model="value" />
    ...
    <div class="new">
      ...
      <transition name="move">
        <div class="new-item-box">
          ...
          >
            <div class="left">
              <i class="iconfont" v-if="item.title=='湿垃圾'" style="color: #d6825d;">&#xe622;</i>
              <i class="iconfont" v-if="item.title=='干垃圾'" style="color: #7da0dd;">&#xe604;</i>
              <i class="iconfont" v-if="item.title=='有害垃圾'" style="color: #f47f75;">&#xe605;</i>
              <i class="iconfont" v-if="item.title=='可回收物'" style="color: #62d1b8;">&#xe627;</i>
            </div>
            <div class="right">
              <div>
                <h2>{{item.title}}</h2>
      ...
      <div class="content">
        <div v-for="item in searchArr" :key="item.gname">
          <div>
            ...
            <p
              v-if="item.gtype=='可回收'"
            >适宜可循环利用的生活废弃物。投放可回收物时，应尽量保持清洁干燥，避免污染；立体包装应该清空内容物，滞后压扁投放；易破损或有裹尖锐边角的应包后投放。</p>
            <p v-if="item.gtype=='有害垃圾'">有害垃圾指对人体健康或者自然环境造成直接或者潜在危害的生活废弃物。常见的有包括废灯管、废油漆、杀虫剂、</p>
            <p v-if="item.gtype=='干垃圾'">干垃圾即其它垃圾，指除可回收物、有害垃圾、湿垃圾以外的其它生活废弃物。常见的有包括废弃的纸张、塑料、玻璃、烟头、 金属</p>
            <p
              v-if="item.gtype=='湿垃圾'"
            >湿垃圾又称为厨余垃圾、有机垃圾，即易腐垃圾，指食材废料、剩菜剩饭、过期食品、瓜皮果核、花卉绿植、中药药渣等易腐的生物质生活废弃物。</p>
          </div>

          <div v-if="item.gtype=='可回收'" class="bg" :class="item.gtype=='可回收'?'kehuishou':''"></div>
          <div v-if="item.gtype=='有害垃圾'" class="bg" :class="item.gtype=='有害垃圾'?'youhai':''"></div>
          <div v-if="item.gtype=='干垃圾'" class="bg" :class="item.gtype=='干垃圾'?'gan':''"></div>
          <div v-if="item.gtype=='湿垃圾'" class="bg" :class="item.gtype=='湿垃圾'?'shi':''"></div>
        </div>
      </div>
    </van-action-sheet>
  </div>
